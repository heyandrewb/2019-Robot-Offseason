...
/**
 * This class uses a Pixy2 set on line follow mode to follow lines
 * with the robot.
 * 
 * Setup:
 *  -Use the Pixy 2 PixyMon program to configure
 *   your Pixy to the mode line follow.
 *  -Use the Pixy_Line_Follower Arduino program on a compatable arduino.
 *  -Hook the Pixy2 to the Arduino over the included SPI connection.
 *  -Hook the Arduino to the RoboRio with I2C.
 * 
 * Instructions:
 *  -Run the constructor, PixyLineFollow() in the robotInit() method.
 *  -Run arduinoRead() method every loop.
 *  -Either get the error and use it yourself or use
 *   a provided drive function to follow a line.
 * 
 * @author Max Green
 */
public class PixyLineFollow {
    ...
    /**
     * m_error is calculated using the X1 from the primary vector
     * returned by the Pixy and finding the distance from the center
     * of the screen. Negative values indicate that the robot must turn
     * left and positive values indicate that the robot must turn right.
     * The farther from zero the error is the more the robot must turn.
     */
    public int m_error;
    ...
    /**
     * This constructor initializes the class with the standard USB port
     * and a basic baud rate of 9600. It sets the BufferSize to 1.
     */
    ...
    /**
     * This constructor initializes the class with the standard USB port
     * and a custom baud rate. It sets the BufferSize to 1.
     * 
    ...
    /**
     * This constructor initializes with a custom SerialPort. 
     * It sets the BufferSize to 1.
     * 
    ...
    /**
     * This method will read the bytes from the Arduino SerialPort and
     * use the bytes to find and update m_error. If error equals 100 then
     * there is no line to track.
     */
    public void arduinoRead()
    {
        ...
        if(output.length == 0)
        {
            // If there are no bytes then set m_error to 100 indicating that Line Follow is not ready.
            m_error = 100;
        }
        else
        {
    ...
    {
        // If error is not equal to 100, the tracking number then the line is being tracked
        if(m_error != 100)
        {
            return true;
        }
    ...
    /**
     * A method that uses the m_error to control the robot. This
     * method uses 2 TalonSRXs.
    ...
    /**
     * A method that uses the m_error to control the robot. This
     * method uses 2 Speed Controllers.
    ...
    /**
     * A method that uses the m_error to control the robot. This
     * method uses 4 Speed Controllers.
     ...
     * @param speed The speed that the function should run at. (0 to 1)
     */
    public void lineFollowTalon(SpeedController leftFront, SpeedController leftBack, SpeedController rightFront, SpeedController rightBack, double speed)
    {
        // Set the base speeds of the function
        double driveRight = speed;
        double driveLeft = speed;
